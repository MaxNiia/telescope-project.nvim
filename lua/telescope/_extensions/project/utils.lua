...
M.get_projects = function(order_by)
	...
	for _, project in pairs(M.get_project_objects()) do
		...
		local git_cmd = "git branch -C " .. project.path .. "--no-color --show-current"
		project.git_branch = tostring(vim.fn.systemlist(git_cmd)[1])
		local git_root_fatal = _utils.string_starts_with(project.git_branch, "fatal")
		if not project.git_branch or git_root_fatal then
			project.git_branch = ""
		end
